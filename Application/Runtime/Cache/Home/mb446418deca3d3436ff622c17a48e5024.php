...
<body>
    ...
    <div class="release">
        <div class="fanhui padding" id='fanhui_1'><a href="<?php echo U('Member/index');?>"><span class="iconfont tb_fanhui"id="shangjia_info_fanhui">&#xe603;</span></a>商品编辑</div>
        <div id="infor" class="infor" style="color: #666;"></div>
 
        <form name="release_goods" method="post" enctype="multipart/form-data" action="<?php echo U('Shop/bianji_check',array(goods_id=>$goods['goods_id']));?>" onsubmit="editor.sync()">
...
<div class='xuxian'></div>
 ...
 <script type="text/javascript">
        ...
        var default_cat_id="<?php echo ($goods['cat_id']); ?>";
        ...
        //商品取值
        $('input[name=title]').val('<?php echo ($goods["goods_name"]); ?>');
        $('textarea[name="goods_jianjie"]').val('<?php echo ($goods["goods_jianjie"]); ?>');
        $('input[name="units"]').val('<?php echo ($goods["units"]); ?>');
        $('input[name="price"]').val('<?php echo ($goods["price"]); ?>');
        $('input[name="yuan_price"]').val('<?php echo ($goods["yuan_price"]); ?>');
        $('input[name="fanxian"]').val('<?php echo ($goods["fanxian"]); ?>');
        
        
         var goods_zhanshitu="<?php echo ($goods[goods_img]); ?>";
   $('input[name=goods_zhanshitu]').attr('value',goods_zhanshitu);
    var goods_img="";
    $('#img_count').html($('#insert_img .div_goods_img').length);
...
<script>
    //商品属性
    <?php if(is_array($goods['shuxing'])): $i = 0; $__LIST__ = $goods['shuxing'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>var shuxing_vo="<?php echo ($vo); ?>";
        $('select[name="shuxing[]"]>option[value='+shuxing_vo+']').attr('selected','selected');<?php endforeach; endif; else: echo "" ;endif; ?>
