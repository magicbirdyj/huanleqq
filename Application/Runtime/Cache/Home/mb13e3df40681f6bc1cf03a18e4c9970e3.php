...
<body>
    ...
    <div class="view">
        <div class="d2">
            ...
            <div class="d2_content"><span>订单编号：<?php echo ($order['order_no']); ?></span> <span>创建时间：<?php echo (date('Y-m-d H：i',$order['created'])); ?> </span><span>收货地址：<?php echo ($order['order_address']); ?></span><span>分享返现：<?php echo ($fenxiang==0?'未分享':'已分享'); ?></span></div>
        ...
        <div class="view_info">
                <?php if(($order['tuan_no'] != 0) and ($order['status'] == 1) and ($order['pay_status'] == 1)): ?><div>当前订单状态：<span class="red">还差 <span class="price_red"><?php echo ($order['count']); ?></span> 人成团</span></div>
                    <?php else: ?>
                        <div>当前订单状态：<span class="red"><?php echo order_status_shops($order['pay_status'],$order['status'],$order['order_id'])['status'];?></span></div><?php endif; ?>
                <?php if($order['status'] == '3'): ?><div>买家确认收货后，您将收到货款</div>
                    <div class="d1">
                        <a  class="button_a_order"  href="<?php echo U('Shop/view_wuliu',array('order_id'=>$order['order_id']));?>">查看物流</a>
                    </div>
                <?php elseif(($order['pay_status'] == '0') and ($order['status'] < '6')): ?>
                    <div>您可以主动联系买家，催促其付款</div>
                    <div class="d1">
                        <a  class="button_a_order quxiao_order" name="<?php echo ($order['order_id']); ?>">取消订单</a>
                    </div>   
                <?php elseif(($order['status'] == 7) or ($order['status'] == 1 and $order['pay_status'] == 1) or ($order['pay_status'] == 2) or ($order['pay_status'] == 3)): ?>
                    <div class="d1">
                        <a  href="<?php echo order_status_shops($order['pay_status'],$order['status'],$order['order_id'])['status_url'];?>" class="button_a_order"><?php echo order_status_shops($order['pay_status'],$order['status'],$order['order_id'])['status_button'];?></a>
                    </div><?php endif; ?>
        </div>
    </div>
